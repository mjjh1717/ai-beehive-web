<!--
 * @Author: mjjh
 * @LastEditTime: 2023-06-26 00:28:41
...
import midjourney from '../cellRoom/midjourney/index.vue'
import { useRoomStore } from '@/store'
import { isEmpty } from '@/utils/common/index'

const roomStore = useRoomStore()
const { roomData } = toRefs(roomStore)
// 判断需要加载的房间类型
</script>

<template>
  <MainPage h-screen>
    <div v-if="isEmpty(roomData) ">
      请选择房间
    </div>
    <div v-else>
      ...
      <midjourney v-if="roomStore.roomData.cellCode === 'Midjourney'" :key="roomStore.roomData.roomId" :room-data="roomStore.roomData" />
    </div>
  </MainPage>
</template>
